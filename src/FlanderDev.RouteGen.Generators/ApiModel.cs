...
internal sealed class ApiParameterModel(string name, string typeFullName)
{
    ...
    /// <summary>
    /// True when this parameter's type formats differently depending on the current culture
    /// (numeric types, DateTime/DateTimeOffset/TimeSpan/DateOnly/TimeOnly). Used only for
    /// <see cref="ParameterKind.Query"/>/<see cref="ParameterKind.RouteOrAuto"/>/<see cref="ParameterKind.Form"/>
    /// parameters, whose values are formatted into the URL/form via <c>ToString()</c>: when true,
    /// the client emitter passes <c>CultureInfo.InvariantCulture</c> explicitly, matching what
    /// ASP.NET Core's built-in query/route/form value providers already assume server-side.
    /// </summary>
